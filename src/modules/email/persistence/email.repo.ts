...
export function mapEntityToAggregate(entity: EmailEntity, emailFactory: EmailFactory): Email<true> {
    return emailFactory.construct(
        entity.id,
        entity.createdAt,
        entity.updatedAt,
        entity.name,
...
export class EmailRepo {
    ...
    public async findById(id: string): Promise<Option<Email<true>>> {
        ...
        return email && mapEntityToAggregate(email, this.emailFactory);
    }
    ...
    public async findByIds(ids: string[]): Promise<Map<string, Email<true>>> {
        ...
        emailEntities.forEach((emailEntity: EmailEntity) => {
            const email: Email<true> = mapEntityToAggregate(emailEntity, this.emailFactory);
    ...
    private async create(email: Email<false>): Promise<Email<true>> {
        ...
        return mapEntityToAggregate(emailEntity, this.emailFactory);
    }
