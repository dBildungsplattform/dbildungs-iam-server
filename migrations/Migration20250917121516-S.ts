...
export class Migration20250917121516 extends Migration {
    public async up(): Promise<void> {
        this.addSql('create schema if not exists "email";');
        this.addSql(
            "create type \"email\".\"email_address_new_status_enum\" as enum ('REQUESTED', 'ACTIVE', 'DEACTIVE', 'UNASSIGNED', 'FAILED');",
        );
        this.addSql(
            'create table "email"."address" ("id" uuid not null, "created_at" timestamptz not null, "updated_at" timestamptz not null, "address" varchar(255) not null, "priority" int not null, "status" "email"."email_address_new_status_enum" not null, "ox_user_id" varchar(255) null, "spsh_person_id" varchar(255) null, "marked_for_cron" timestamptz null, constraint "address_pkey" primary key ("id"));',
        );
