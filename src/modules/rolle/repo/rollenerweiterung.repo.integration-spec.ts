...
describe('RollenerweiterungRepo', () => {
    ...
    describe('deleteByRolleId', () => {
        let organisation: Organisation<true>;
        let otherOrganisation: Organisation<true>;
        let rolle: Rolle<true>;
        let otherRolle: Rolle<true>;
        let serviceProvider: ServiceProvider<true>;
        let otherServiceProvider: ServiceProvider<true>;
        ...
        beforeEach(async () => {
            ...
            if (otherRolleOrError instanceof DomainError) {
                throw new Error('Failed to create Rolle');
            }
            otherRolle = otherRolleOrError;
        ...
        it('should delete all rollenerweiterungen for a rolle and keep others', async () => {
            await sut.create(factory.createNew(organisation.id, rolle.id, serviceProvider.id));
            await sut.create(factory.createNew(otherOrganisation.id, rolle.id, serviceProvider.id));
            await sut.create(factory.createNew(otherOrganisation.id, rolle.id, otherServiceProvider.id));
            await sut.create(factory.createNew(organisation.id, otherRolle.id, serviceProvider.id));

            await sut.deleteByRolleId(rolle.id);
            ...
            await expect(
                sut.exists({
                    organisationId: organisation.id,
                    rolleId: otherRolle.id,
                    serviceProviderId: serviceProvider.id,
                }),
            ).resolves.toBe(true);
        });

        it('should be a no-op when no rollenerweiterung exists for the rolle', async () => {
            await sut.create(factory.createNew(organisation.id, otherRolle.id, serviceProvider.id));

            await expect(sut.deleteByRolleId(rolle.id)).resolves.toBeUndefined();
            await expect(
                sut.exists({
                    organisationId: organisation.id,
                    rolleId: otherRolle.id,
