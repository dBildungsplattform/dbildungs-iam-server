...
export class VidisApiService {
    ...
    private static readonly PATH_GET_ACTIVATED_ANGEBOTE_BY_SCHOOL = (vidisSchoolId: string): string => `/o/vidis-rest/v1.0/offers/activated/by-school/${vidisSchoolId}`;
    ...
    public async getActivatedAngeboteBySchool(kennung: string): Promise<Result<VidisServiceResponseAngebot []>> {
        try{
            ...
            const response: AxiosResponse<VidisApiResponse<VidisApiResponseAngebotBySchool>> = await firstValueFrom(
                this.httpService.get(this.constructUrl(VidisApiService.PATH_GET_ACTIVATED_ANGEBOTE_BY_SCHOOL(this.convertKennungToVidisSchoolId(kennung))), {
                    headers: { Authorization: `Bearer ${token}` },
                }),
            );
            if(response.status < 200 || response.status >= 300) {
