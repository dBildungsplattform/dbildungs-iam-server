...
describe('ServiceProviderRepo', () => {
    ...
    describe('findBySchulstrukturknoten', () => {
        it('should return the service provider', async () => {
            ...
            const sp: ServiceProvider<false> = DoFactory.createServiceProvider(false, { providedOnSchulstrukturknoten });
            const persistedServiceProvider = await sut.save(sp);
            const result: Array<ServiceProvider<true>> = await sut.findBySchulstrukturknoten(providedOnSchulstrukturknoten);
